...
- Fixed flaky tests that mocked the `Log` facade (`Log::shouldReceive(...)->once()`) under PHP 8.4: old vendor code (`Illuminate\Log\Logger`, an Orchestra exception class) triggers implicit-nullable-parameter deprecation notices on PHP 8.4, and whatever routes those through the logger in CI's resolved dependency set left the `Log` facade's internal Mockery state such that even `Log::spy()` (tried first) failed with "Call to a member function warning() on null". Replaced both tests with `Event::fake(MessageLogged::class)` + `Event::assertDispatched(...)`, which asserts on Laravel's real log-dispatched event instead of mocking the facade, sidestepping the whole class of Facade/Mockery state issues. Verified against the full Laravel 10-13 x prefer-lowest/prefer-stable matrix under both PHP 8.3 and PHP 8.4 locally.
